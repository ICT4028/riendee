...
# ฟังก์ชันคุยกับ Gemini ผ่าน REST API (ไม่ต้องใช้ google-generativeai library)
def generate_gemini_content(api_key, prompt_text):
    ...
    if response.status_code == 200:
        ...
        try:
            ...
            return "ไม่สามารถดึงข้อมูลคำตอบจาก JSON โครงสร้างระบบได้"
    else:
        return f"เกิดข้อผิดพลาดจาก Google API (Status {response.status_code}): {response.text}"
...
# เงื่อนไขการแสดงผล UI ตามโหมดที่ผู้ใช้เลือก
if input_method == "📝 พิมพ์/วางข้อความเอง":
    ...
    form_question = st.text_area("❓ ตัวโจทย์/คำถามภาษาอังกฤษ (Question):", placeholder="พิมพ์คำถามของคุณตรงนี้ (หากมีหลายคำถามย่อย ให้พิมพ์แยกข้อมาได้เลย เช่น 1)... 2)...)")
    ...
    quiz_topic = st.text_input("📝 พิมพ์หัวข้อเรื่องหรือบทเรียนที่ต้องการทำข้อสอบ:", placeholder="เช่น English vocabulary for daily life, กฎแรงดึงดูดของนิวตัน")
            ...
            try:
                with st.spinner('riendee AI กำลังประมวลผลข้อมูลให้อย่างใส่ใจ...'):
                    if input_method == "🎯 ให้ AI ออกข้อสอบ 4 ตัวเลือกให้ฉันฝึกทำ":
                        ...
                        ⚠️ กฎเหล็กในการตอบคำถาม (สำคัญที่สุด):
                        1. ตรวจสอบว่าในเนื้อหาหรือโจทย์ที่ส่งมา มีคำถามทั้งหมดกี่ข้อหรือกี่ประเด็น
                        2. คุณต้องตอบเรียงลำดับเป็นข้อๆ (ข้อ 1, ข้อ 2, ข้อ 3...) ตามลำดับคำถามที่ผู้ใช้ถามมาอย่างเคร่งครัด ห้ามรวบคำตอบเด็ดขาด
                        3. ในแต่ละข้อ ให้ประกอบด้วยโครงสร้างดังนี้:
                           - 📝 [บทแปลภาษาไทยของคำถามข้อนั้น]
                           - 💡 [คำตอบหรือเฉลยที่ถูกต้องที่สุด]
                           - 🔬 [การอธิบายเหตุผลหรือวิธีคิดทีละสเต็ปสั้นๆ เข้าใจง่าย]
                        
                        ตอบกลับด้วยภาษาไทยที่เป็นระเบียบ สุภาพ และใช้ Markdown ในการจัดหัวข้อให้ชัดเจนอ่านง่าย
                        
                        นี่คือข้อความที่ต้องประมวลผล:
                        "{user_input}"
                        """
                    
                    # เรียกใช้ฟังก์ชัน API ตรง
                    ai_response = generate_gemini_content(api_key, prompt)
                    
                    # แสดงกล่องการ์ดผลลัพธ์
                    st.markdown('<div class="result-card">', unsafe_allow_html=True)
                    if input_method == "🎯 ให้ AI ออกข้อสอบ 4 ตัวเลือกให้ฉันฝึกทำ":
                        st.markdown(f"### 📝 ชุดข้อสอบจำลองหัวข้อ {quiz_topic} โดย riendee")
                    else:
                        st.markdown("### ✨ ผลการวิเคราะห์แยกตามข้อโดย riendee")
